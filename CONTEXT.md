...
* **`HomeScreen`:** Displays a dynamic banner, runner's pass dashboard, countdown timer, quick actions grid, emergency SOS buttons, and weather advisors.
  ...
  * **"Ficha de Corredor" (Runner's Pass Card):** Displays a dynamic card if a linked participant is stored in the Hive cache.
    ...
    * **Dorsal/Plate Visibility:** If the participant has a valid plate (`nroPlaca != "0"` and not empty), it shows their bib number in a highly contrasted dorsal box and marks the status as `INSCRIPCIÓN ACTIVA` (green). If they don't have a plate (`nroPlaca == "0"` or empty), it hides the dorsal box and shows `PAGO PENDIENTE` (amber).
...
* **`MoreScreen`:** Embeds contact messages, application sharing, and brand info. Contact options such as WhatsApp support a visual opacity filter when inactive. The **"Acerca de"** section displays the custom logo asset and email contact link inside a compact card, while the version label is rendered as a clean footer at the bottom of the screen. It conditionally exposes a **"Cerrar App"** outlined action button only on Android targets (fully hidden on iOS for App Store guideline compliance).
