...
* **`KitAuthorizationScreen`:** Displays kit collection options for the runner.
  ...
  * **Option Selection:** The choices are card-based selections: *"Te presentás a retirar tu kit"* and *"Autorizá a un tercero a retirar tu kit"*.
...
* **`MoreScreen`:** Embeds contact messages, application sharing, and brand info. Contact options such as WhatsApp support a visual opacity filter when inactive. WhatsApp details include the `CONTACTO_MENSAJE_WHATSAPP` text displayed below the phone number (structured similarly to the email card's response notice), and the phone number is displayed with a legible phone mask (e.g. `+54 9 381 408-3349` for Argentine numbers). The **"Acerca de"** section displays the custom logo asset and email contact link inside a compact card, while the version label is rendered as a clean footer at the bottom of the screen, displaying the full version including the build number in the standard `version+buildNumber` format (e.g. `1.0.0+2`). It conditionally exposes a **"Cerrar App"** outlined action button only on Android targets (fully hidden on iOS for App Store guideline compliance).
