...
## 8. Participant & Category REST API Updates

Participant update endpoints in `ParticipantApiService` are refactored to remove the `{partiId}` path parameters where applicable, and a single unified update endpoint is added. Category entities map the `categEspecial` (special category, value `1`) field:
